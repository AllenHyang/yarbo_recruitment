...
export default function CampusJobDetailPage() {
  ...
  const [job, setJob] = useState<CampusJobDetail | null>(null);
  ...
  useEffect(() => {
    const fetchJobDetail = async () => {
      ...
      try {
        ...
        if (data.success) {
          // 检查是否为校园招聘职位
          if (data.job_category !== 'campus') {
            setError('该职位不是校园招聘职位');
            return;
          }

          // 转换数据格式
          const jobDetail: CampusJobDetail = {
            id: data.id,
            title: data.title,
            department: data.department,
            location: data.location,
            type: data.employment_type || '全职',
            level: data.experience_level === 'entry' ? '应届生' : data.experience_level,
            salary_display: data.salary_display || `${data.salary_min || 0}-${data.salary_max || 0}K`,
            ...
            campus_specific: data.campus_specific || false,
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-gray-600 mb-6">{error || '未找到该校园招聘职位'}</p>
