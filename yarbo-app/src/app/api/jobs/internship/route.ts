...
export async function GET(request: Request) {
  try {
    ...
    const queryParams = new URLSearchParams({
      select: '*',
      status: 'eq.active',
      job_category: 'eq.internship' // 筛选实习职位
    });
    ...
    if (duration && duration !== 'all') {
      queryParams.append('duration', `eq.${duration}`);
    }
    if (search) {
    ...
    if (!response.ok) {
      console.error('查询实习职位失败:', response.statusText);
      return new Response(JSON.stringify({
        success: false,
        error: '查询实习职位失败'
      }), {
    ...
    console.error('实习职位 API 错误:', error);
    return new Response(JSON.stringify({
      success: false,
      error: '服务器内部错误',
